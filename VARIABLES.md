...
A list of files within the project which the contain variables that need to be changed. The Encryption column denotes if the variable will be encrypted (tick), or stored in plaintext (blank). SSL keys are expected to be generated by openssl library (ssh-keygen generated keys might not work for Lets Encrypt).
...
| Variable | Description | Encrypted |
| ------ | ----------- | ----------- |
| `cluster_credential_git_repo`  | SSH URL of the Github credential repository to be used (fork of this) |  |
| `cluster_credential_bundle_aws_name` | AWS cluster credential bundle name (some from aws_credentials_list) |  |
...
Private key to be used with Lets Encrypt. Be sure to use openssl when generating the key (ssh-keygen might not work).
...
| Variable | Description | Encrypted |
| ------ | ----------- | ----------- |
| `tower_github_scm_password` | The password for the private key below | ✔ |
